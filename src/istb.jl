...
"""
function update_ista_mat!(B::AbstractArray{Float64,2}, 
...
function update_ista_mat!(B::AbstractArray{Float64,2}, 
     ...
     b2update = B .- stepsize[1].*(grad./norms) # L2 update
     b2sign = sign.(b2update) # L2 update sign
...
"""
function istb!(X::AbstractArray{Float64,2}, Y::AbstractArray{Float64,2}, 
    ...
    while (abs(log(crit/oldcrit)) > thresh) && (iter < maxiter)
        ...
        update_ista_mat!(B, resid, grad, X, Y, Z, norms, lambda, reg, stepsize) 
