...
"""
function update_istc(B::AbstractArray{Float64,2}, 
...
"""
function istc!(X::AbstractArray{Float64,2}, Y::AbstractArray{Float64,2}, 
    ...
    while (abs(log(crit/oldcrit)) > thresh) && (iter < maxiter)
        ...
        B = update_istc(B, resid, grad, X, Y, Z, norms, lambda, reg, stepsize) 
        calc_resid!(resid, X, Y, Z, B) 
